package com.hooni.encrypt;

public class Aes
{

	public Aes(){}
	
	public int[] cipher(int[] input, int[][] w)
	{
		int Nb = 4;
		int Nr = w.length/Nb -1;
		int state[][] = new int[Nb][4];
		
		for (int i = 0; i< 4*Nb; i++)
		{
			state[i%4][(int)Math.floor(i/4)] = input[i];
		}
		
		addRoundKey(state, w, 0, Nb);
		
		for (int round=1; round<Nr; round++)
		{
			subBytes(state,Nb);
			shiftRows(state,Nb);
			mixColumns(state,Nb);
			addRoundKey(state,w,round,Nb);
		}
		subBytes(state,Nb);
		shiftRows(state,Nb);
		addRoundKey(state,w,Nr,Nb);
		
		int output[] = new int[4*Nb];
		for(int i=0; i<4*Nb; i++)
		{
			output[i] = state[i%4][(int)Math.floor(i/4)];
		}
		return output;
	}
	
	public int[][] keyExpansion(int key[])
	{
		int Nb = 4;
		int Nk = key.length/4;
		int Nr = Nk + 6;
		
		int w[][] = new int[Nb*(Nr+1)][Nb];
		int temp[] = new int[4];
		
		for(int i=0; i <Nk; i++)
		{
			int r[] = {key[4*i],key[4*i+1],key[4*i+2],key[4*i+3]};
			w[i]=r;
		}
		
		for (int i=Nk; i<(Nb*(Nr+1)); i++)
		{
			w[i] = new int[4];
			for (int t=0; t<4; t++)
			{
				temp[t] = w[i-1][t];
			}
			if (i % Nk == 0)
			{
				rotWord(temp);
				subWord(temp);
				for (int k=0; k<4; k++)
				{
					temp[k] ^= rCon[i/Nk][k];
				}
			}
			else if (Nk > 6 && i%Nk ==4)
			{
				subWord(temp);
			}
			for (int t=0; t<4; t++)
			{
				w[i][t] = w[i-Nk][t] ^ temp[t];
			}
		}
		return w;
	}
	
	private void subBytes(int[][] state, int Nb)
	{
		for (int r=0; r<4; r++)
			for (int c=0; c<Nb; c++)
				state[r][c] = sBox[state[r][c]];
	}
	
	
	
	private void shiftRows(int state[][], int Nb)
	{
		int t[] = new int[4];
		for (int r=1; r<4; r++)
		{
			for (int c=0; c<4; c++) t[c] = state[r][(c+r)%Nb];
			for (int c=0; c<4; c++) state[r][c] = t[c];
		}
	}
	
	private void mixColumns(int state[][], int Nb)
	{
		for (int c=0; c<4; c++)
		{
			int a[] = new int[4];
			int b[] = new int[4];
			for (int i=0; i<4; i++)
			{
				a[i] = state[i][c];
				int temp = a[i] & 0x80;
				if (temp !=0)
				{
					b[i] = state[i][c] << 1 ^ 0x011b ;
				}
				else
					b[i] = state[i][c] << 1;
				//b[i] = ((a[i]&0x80) == 1 ? state[i][c]<<1 ^ 0x011b : state[i][c] << 1);
			}
			// a[n] ^ b[n] is a?{03} in GF(2^8)
		    state[0][c] = b[0] ^ a[1] ^ b[1] ^ a[2] ^ a[3]; // 2*a0 + 3*a1 + a2 + a3
		    state[1][c] = a[0] ^ b[1] ^ a[2] ^ b[2] ^ a[3]; // a0 * 2*a1 + 3*a2 + a3
		    state[2][c] = a[0] ^ a[1] ^ b[2] ^ a[3] ^ b[3]; // a0 + a1 + 2*a2 + 3*a3
		    state[3][c] = a[0] ^ b[0] ^ a[1] ^ a[2] ^ b[3]; // 3*a0 + a1 + a2 + 2*a3
		}
	}
	
	private void addRoundKey(int state[][], int[][] w, int rnd, int Nb)
	{
		for (int i=0; i<4; i++)
		{
			for (int j=0; j<Nb; j++)
			{
				state[i][j] = state[i][j] ^ w[rnd*4+j][i];
			}
		}
	}
	
	private void subWord(int[] w)
	{
		for (int i=0; i<4; i++)
			w[i] = sBox[w[i]];
	}
	private void rotWord(int[] w)
	{
		int tmp = w[0];
		for (int i=0; i<3; i++) w[i] = w[i+1];
		w[3] = tmp;
	}
	
	private static int[] sBox = {};
	private static int[][] rCon = 
		{{0x00, 0x00, 0x00, 0x00},
		{0x01, 0x00, 0x00, 0x00},
		{0x02, 0x00, 0x00, 0x00},
		{0x04, 0x00, 0x00, 0x00},
		{0x08, 0x00, 0x00, 0x00},
		{0x10, 0x00, 0x00, 0x00},
		{0x20, 0x00, 0x00, 0x00},
		{0x40, 0x00, 0x00, 0x00},
		{0x80, 0x00, 0x00, 0x00},
		{0x1b, 0x00, 0x00, 0x00},
		{0x36, 0x00, 0x00, 0x00}};
	
}
